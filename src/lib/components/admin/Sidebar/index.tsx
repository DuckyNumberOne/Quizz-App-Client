...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    <aside
      ref={sidebar}
      className={` fixed w-[11%] left-0 top-0 z-9999 flex h-screen flex-col overflow-y-hidden bg-white shadow-2 duration-300 ease-linear dark:bg-boxdark lg:translate-x-0 ${
        ...
        <nav className="mt-5 px-4 py-4 lg:mt-9 lg:px-6">
          ...
          <div>
            ...
            <ul className="mb-6 flex flex-col gap-1.5">
              <li>
                <button
                  className={`w-full group relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-medium text-black duration-300 ease-in-out hover:bg-slate-200 dark:hover:bg-meta-4 ${
                    pathname.includes("calendar") &&
                    "bg-slate-200 dark:bg-meta-4"
                  }`}
          ...
          <ButtonDefault
            ...
            className="transition-colors duration-200 ease-in-out flex items-center justify-center px-7.5 py-2.5 text-lg font-semibold mb-1 h-10 rounded-lg primary relative min-w-max w-full bg-[#8854c0] text-[#fdfcfe] shadow-[0_4px_#6c4298] hover:bg-[#8854c0]"
          />
        </nav>
        {/* <!-- Sidebar Menu --> */}
      </div>
